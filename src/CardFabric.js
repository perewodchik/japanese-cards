...
export class CardFabric extends React.Component {
    ...
    unfocus = () => {
        for(var i in this.cardRefs) {
            this.cardRefs[i].current.unblur();
        }
        document.getElementById("blocker").style.visibility = "hidden";
        this.cardRefs[this.state.focusedCardIndex].current.unfocus();
    }
    ...
    render() {
        return (
            <div id="CardFabric">
                {this.cardComponents}
                <div onClick={this.unfocus} id="blocker"></div>
            </div>
        );
    }
